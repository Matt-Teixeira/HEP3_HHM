...
async function ge_cv_sys_error(filePath) {
  ...
  try {
    const sys_error = /(?<sequencenumber>.+?),(?<date>.+?),(?<time>.+?),(?<subsystem>.+?),(?<errorcode>.+?),(?<errortext>.+?),(?<exam>.+?),(?<exceptioncategory>.+?),(?<application>.+?),(?<majorfunction>.+?),(?<minorfunction>.+?),(?<fru>.+?),(?<viewinglevel>.+?),(?<rootcause>.+?),(?<repeatcount>.+?),(?<debugtext>".+"?|.+?),(?<sourcefile>.+?),(?<sourceline>.+)/
    ...
    const mappedData = mapDataToSchema(data, ge_cv_sys_error_schema);
