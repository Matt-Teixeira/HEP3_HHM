...
const {
  ...
  updateTable,
  insertData,
} = require("../../../utils/phil_mri_monitor_helpers"); //cryo_comp_press_alarm

async function cryo_comp_press_alarm(sme, data) {
  try {
    ...
    for await (const obs of data) {
      ...
      if (currentDate === prevData) {
        bucket.push(obs.cryo_comp_press_alarm);
      ...
      if (currentDate !== prevData) {
        ...
        if (systemDates.includes(prevData)) {
          ...
          await updateTable("cryo_comp_press_alarm", [maxValue, sme, prevData]);
          bucket = [];
          prevData = obs.host_date;
          bucket.push(obs.cryo_comp_press_alarm);
        } else {
          // If date dose not exist: INSERT new row
          await insertData("cryo_comp_press_alarm", [sme, prevData, maxValue]);
          bucket = [];
          prevData = obs.host_date;
          bucket.push(obs.cryo_comp_press_alarm);
        }
    ...
    if (systemDates.includes(prevData)) {
      ...
      await updateTable("cryo_comp_press_alarm", [
        maxValue,
        sme,
      ...
      await insertData("cryo_comp_press_alarm", [
        sme,
        data[data.length - 1].host_date,
        maxValue,
      ]);
...
module.exports = cryo_comp_press_alarm;
