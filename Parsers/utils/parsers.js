...
const ge_re = {
  gesys_mroc: {
    ...
    no_box:
      /SR\s(?<sr_group>\d+)[\n\r](?<time_stamp>\d+)\s+(?<num_1>\d+)\s+(?<num_2>\d+)\s+\w+\s(?<month>\w+)\s+(?<day>\d+)\s(?<time>\d{1,2}:\d{1,2}:\d{1,2})\s(?<year>\d+)\s+(?<num_3>(-)?\d+)\s(?<num_4>(-)?\d+)\s+mroc\s(?<mroc>.*)[\n\r](?<data_1>.*?)\s+(?<num_5>\d+)[\n\r]\s(?<data_2>(.+)((\r?\n.+)*))[\n\r]\s?EN\s(?<en>\d+)/,
    box:
      /SR\s(?<sr_group>\d+).*\s+(?<time_stamp>\d+)\s+(?<num_1>\d+)\s+(?<num_2>\d+)\s+\w+\s(?<month>\w+)\s+(?<day>\d+)\s(?<time>\d{1,2}:\d{1,2}:\d{1,2})\s(?<year>\d+)\s+(?<num_3>\d+)\s(?<num_4>\d+)\s+mroc\s(?<mroc>.*)\s+(?<data_1>.*?)\s+(?<num_5>\d+)\s+Server\sName:\s(?<server_name>\w+)\s+\s+(?<data_2>.*)\s+EN\s(?<en>.*)/,
    exception_class:
